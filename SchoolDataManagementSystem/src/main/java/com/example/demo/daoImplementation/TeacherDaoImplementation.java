...
import com.example.demo.exceptionalHandling.EntityAlreadyExistsException;
...
public class TeacherDaoImplementation implements TeacherDao {
	...
	public Teacher saveTeacher(Teacher s) {
		if (repository.existsByEmail(s.getEmail())) {
			throw new EntityAlreadyExistsException("TEacher is already present with the email " + s.getEmail());
		}
		if (repository.existsByPhoneNo(s.getPhoneNo()))
			throw new EntityAlreadyExistsException("TEacher is already present with the number " + s.getPhoneNo());
	...
	public Teacher updateTeacher(Teacher s, int id) {
		...
		if (optional.isPresent()) {
			if (optional.get().getId() != id) {
				throw new EntityAlreadyExistsException("Student is already present with the phone " + s.getPhoneNo());
			}
		}
		s.setId(id);
		return repository.save(s);
